(function () {
  ...
  function init() {
    Object.assign(els, {
      ...
      openFolderFallbackBtn: $('openFolderFallbackBtn'),
      ...
      welcomeFallbackBtn: $('welcomeFallbackBtn'),
  ...
  function bindEvents() {
    ...
    els.openFolderFallbackBtn.addEventListener('click', () => els.folderFallbackInput.click());
    els.welcomeFallbackBtn.addEventListener('click', () => els.folderFallbackInput.click());
  ...
  async function openWorkspace() {
    if (!supportsFsAccess) {
      toast('Direct folder editing is not available in this browser. Use “Fallback import folder”.', 'error');
    ...
    try {
      ...
      if (projects.length === 1) await loadProject(projects[0]);
      else showProjectModal(projects);
    } catch (error) {
      if (error && error.name === 'AbortError') return;
      log('error', 'workspace.open_failed', { message: error.message });
      toast('Could not open workspace: ' + error.message, 'error');
    }
  }
  ...
  async function importFolderFallback(event) {
    ...
    updateWorkspaceUi();
    populateProjectPicker();
    log('info', 'workspace.imported_fallback', { name: topName, files: files.length, projects: state.projects.map((item) => item.name) });

    if (!state.projects.length) {
      toast('No conference.yaml found in the imported folder.', 'error');
      return;
    }
    if (state.projects.length === 1) await loadProject(state.projects[0]);
    else showProjectModal(state.projects);
  }
  ...
  function updateWorkspaceUi() {
    els.workspaceName.textContent = state.workspaceName || 'Not opened';
  }

  function updateProjectUi() {
    const loaded = Boolean(state.config);
    els.projectName.textContent = loaded ? state.projectName : 'Select a conference folder';
    if (state.projectName) els.projectPicker.value = state.projectName;
